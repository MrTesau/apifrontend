...
import ApexBar from "components/ReactTable/ApexBar.js";
...
export default function Dashboard(props) {
  ...
  const [selectedDimension, changeDimension] = React.useState(items[5]);
 
  
 // Goal is to update childs state (via call to setState) from parent

  // define setState Functions here
  // pass into ApexChild and call there

  // Or integrate ApexBar into this component, handle everything here

  //fcc ref
  // tag apexComponent with ref
  // create ref here too
  const handleClick = () => {
    let series = [{
      name: selectedDimension.title,
      data: selectedDimension.data.series[0]
    }]
    this.superheroElement.current.changeName(series);
  };
  return (
    <div >
      {alert}
  ...
  {dimensions.map(dimension =>
   ...
   <ApexBar   
    ...
    <div style={{textAlign: "center", alignItems: "center", }}>
      <Button round size="sm" style={{fontFamily: "arial",color:"black", background: "white"}}>
